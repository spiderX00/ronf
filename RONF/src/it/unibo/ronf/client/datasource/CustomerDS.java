...
public class CustomerDS extends DataSource {
	...
	public CustomerDS(String id, final TabCustomer tabCustomer) {
		...
		DataSourceTextField fiscalCodeField = new DataSourceTextField(
				"fiscalCode", "Cod. Fiscale");
		fiscalCodeField.setRequired(true);

		DataSourceTextField docNumberField = new DataSourceTextField(
				"docNumber", "Documento n.");
		docNumberField.setRequired(true);

		setFields(idField, nameField, surnameField, ageField, fiscalCodeField,
				docNumberField);
		...
		customerService.findAll(new AsyncCallback<List<Customer>>() {
			...
			public void onSuccess(List<Customer> result) {
				...
				for (Customer p : result) {
					customerRecord[i] = new GridRecord(p.getId(), p.getName(),
							p.getSurname(), p.getAge(), p.getFiscalCode(), p
									.getDocNumber());
					i++;
